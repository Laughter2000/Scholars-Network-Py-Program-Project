...
def home():
    ...
    welcome_note.pack(side = TOP, pady = 20, fill = BOTH, expand = True)
    ...
    btn_add_contacts.pack(side = TOP, pady = 20)
    ...
    btn_contacts.pack(side = TOP, pady = 20)

    # A column that shows id(#), name, address, phone number, email and actions 
    btn_view_contacts =  Button(HomeFrame, text = "#\t\tName\t\t\tAddress\t\t\tPhone Number\t\t\tEmail Address\t\t\tActions", font=('arial', 12, 'bold'), command= login)
...
def contact_details():
    ...
    if contacts:
        ...
        btn_delete = Button(HomeFrame, text = "Delete", font = ('arial', 8), bg = "red", command = delete_warning)
        btn_delete.pack(side = RIGHT)

        #Update button to update contacts by invoking the delete_warning() function 
        btn_update = Button(HomeFrame, text = "Update", font = ('arial', 8), bg = "orange", command = update_warning)
...
def update_contacts():
    ...
    if name.get() == "" and phone_number.get() == "":
        ...
        cursor.execute("SELECT * FROM contacts WHERE `name` = ? and `phone_number` = ? and `address` = ? and `email_address` = ? and `user_id` = ?", (name.get(), phone_number.get(), id_number))
        if cursor.fetchone() is not None:
            lbl_result3.config(text="Contact Name or Phone Number Already Exists!", fg="red")
        else:
